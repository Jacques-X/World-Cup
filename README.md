...
A private five-player World Cup group-stage predictor built with Next.js, Vercel, and Supabase.

## Features

- Private invite-token access with a signed, HTTP-only cookie
...
Use the first random value for `POOL_ACCESS_TOKEN`, the second for `SESSION_SECRET`, and the bcrypt output for `ADMIN_PASSWORD_HASH`.
...
Open `http://localhost:3000/api/access?token=YOUR_POOL_ACCESS_TOKEN` to establish participant access. The settings tab accepts the unhashed admin password used when generating `ADMIN_PASSWORD_HASH`.
...
4. Share `https://YOUR_DOMAIN/api/access?token=YOUR_POOL_ACCESS_TOKEN` privately.

`SUPABASE_SERVICE_ROLE_KEY`, `POOL_ACCESS_TOKEN`, `ADMIN_PASSWORD_HASH`, and `SESSION_SECRET` are server-only. Never prefix them with `NEXT_PUBLIC_`.

## Database security

All application tables have Row Level Security enabled with no public policies. Browser clients cannot query Supabase directly. Next.js route handlers validate the signed session, enforce admin-only operations, reject cross-origin mutations, and then use the service-role client server-side.
